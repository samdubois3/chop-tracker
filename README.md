...
**Tiebreak:** lowest two-week total, then fewest cumulative points from week 3
through the end of that window. Warm-up weeks are excluded, since those points
don't carry forward anywhere else either.

One consequence worth knowing: in the *first* chop window (W3+4), "points since
week 3" is the same number as the two-week total, so it cannot break a tie
there. A W3+4 tie always falls through to `UNRESOLVED TIE`. That's intentional —
under this league's rules there is no other scoring data that counts yet.

If teams are still tied, the script prints an `UNRESOLVED TIE` warning rather
than guessing — you break it.
